...
#
#	- In convert_to_raw(): Corrected a wrong serialization of empty strings. This returned a byte array instead of an empty string.
...
class Serializer(object):
	...
	def convert_to_raw(number, value, length):
		...
		match opt_type:
			case defines.INTEGER if length == 0:
				return 0

			case defines.STRING if length == 0:
				return ''
			case defines.STRING if isinstance(value, bytes):
				return value.decode("utf-8")

			case defines.OPAQUE if length == 0:
				return bytes()
